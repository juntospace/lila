// Parser for Banco General checking account statement files in Edge Function.

import * as XLSX from "xlsx";

import type { BgParsedStatement, BgStatementRow } from "../types.ts";
import {
...
export function parseBgStatement(
  ...
  if (Array.isArray(workbookOrSheet)) {
    ...
    const wb = workbookOrSheet;
    const name = wb.SheetNames[0];
    layoutTitle = name || "";
    sheet = wb.Sheets[name];
  } else {
    sheet = workbookOrSheet;
  }

// Deterministic Reconciliation Engine for Banco General · CREDICLARO, S.A. (CCBG v2).
...
import { areAmountsEqual, round2 } from "./parsers/utils";
import { consolidateStatements } from "./consolidation";
...
const PANAMA_HOLIDAYS: Set<string> = new Set(); // ISO date strings if needed
...
export function reconcileBancoGeneral(
  ...
  // 2. Classify Statement Movements
  ...
  // Retries and double debits detection
  ...
  for (const fl of allBatchDates) {
    ...
    for (const dt of dts) {
      ...
      // Match Reversals (1 to MAX_REVERSAL_PARTS)
    ...
    // Provisional batches from unmatched reversals
    for (const r of revs) {
    ...
    // Provisional batches from unmatched credits
  ...
  // Unconsumed Yappy processed transactions
  ...
  // 5. Voluntary Inflows & Heuristics
  ...
  // Recurrence heuristics by counterpart

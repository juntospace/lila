// Formatters and translations (English <-> Spanish) for Banco General reconciliation.
// Keeps database and business logic in clean English while providing Spanish labels for UI and reports.
...
export function fromSpanishBatchStatus(esStatus: string): BgBatchStatus {
  ...
  if (norm === "LIQUIDADA_SIN_RECHAZOS" || norm === "LIQUIDADA SIN RECHAZOS") return "settled_no_reversals";
  if (norm === "PENDIENTE") return "pending";
  return "anomaly";
}
...
export function fromSpanishItemStatus(esStatus: string): BgItemStatus {
  ...
  if (norm === "CONFIRMADO" || norm === "CONFIRMADA" || norm === "REALIZADA") return "confirmed";
  return "pending";
}
...
export function fromSpanishYappyStatus(esStatus: string): BgYappyStatus {
  const norm = esStatus.trim().toUpperCase();
  if (norm === "RECIBIDO" || norm === "RECIBIDA") return "received";
  if (norm.startsWith("EN TRANS")) return "in_transit";
  if (norm === "PENDIENTE") return "pending";
  if (norm === "ANOMALIA" || norm === "ANOMALÍA") return "anomaly";
  return "other";
}
...
export function fromSpanishCategory(esCat: string | null | undefined): BgAssignmentCategory | null {
  ...
  const norm = esCat.trim().toUpperCase();
  if (norm === "PRESTAMO" || norm === "PRÉSTAMO" || norm === "LOAN") return "loan";
  if (norm === "NO_PRESTAMO" || norm === "NO_PRÉSTAMO" || norm === "NO PRESTAMO" || norm === "NON_LOAN") return "non_loan";
  if (norm === "OTRO" || norm === "OTHER") return "other";
  return null;
}
...
// -------------------------------------------------------------
// Canonical Contract Export (Matches reference conciliacion.json exactly)
// -------------------------------------------------------------
